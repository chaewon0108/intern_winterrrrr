...
class Detector(object):
    ...
    def detect_faces(self, frame, threshold=0.5, **face_model_kwargs): #TODO : 여기서 224*224를 안 하면~
        ...
        if "img2pose" in self.info["face_model"]: #얼굴 후보군 찾기
            frame = frame / 255
            faces, poses = self.face_detector(frame,**face_model_kwargs)
        else:
            faces = self.face_detector(frame,**face_model_kwargs)

        # TODO: threshold를 위한 디버깅 코드
        # print("\n\n**threshold : ", threshold)
        # print("\n================ [DEBUG: 얼굴 점수 확인] ================")
        # if is_list_of_lists_empty(faces):
        #     print("❌ 모델이 1차 심사에서 '얼굴 같아 보이는 게 단 하나도 없다'고 판단했습니다.")
        #     print("   (이 경우엔 threshold를 아무리 낮춰도 소용없습니다. 강제 좌표 생성이 답입니다.)")
        # else:
        #     for i, fframe in enumerate(faces):
        #         print(f"📸 Frame {i} 분석 결과:")
        #         for j, fface in enumerate(fframe):
        #             # fface[4]가 바로 AI가 매긴 '확률 점수'입니다.
        #             score = fface[4] 
        #             print(f"   - 후보 {j+1}: 점수 {score:.4f} ({(score*100):.1f}%)")
                    
        #             if score < threshold:
        #                 print(f"     👉 [탈락] 현재 기준({threshold})보다 낮아서 버려질 예정입니다.")
        #             else:
        #                 print(f"     👉 [합격] 기준을 통과했습니다.")
        # print("========================================================\n")

        if is_list_of_lists_empty(faces): #얼굴 아예 못 찾은 경우 
        ...
        for fframe in faces:  # first level is each frame
            ...
            for fface in fframe:  # second level is each face within a frame
                if fface[4] >= threshold:  # set thresholds #여기서 threshold 사용됨..,,
                    fframe_x.append(fface)
            thresholded_face.append(fframe_x)

        return thresholded_face

    def detect_landmarks(self, frame, detected_faces, **landmark_model_kwargs): #우리는 landmark 안 씀

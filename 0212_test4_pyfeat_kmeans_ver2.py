...
def remove_emotion_noise_v2(df, min_anchor_len=4, max_noise_len=3,
    ...
    df['noise_esti_score'] = df['esti_score'].copy()
    df['is_noise'] = 'F'
    # is_detected가 False(얼굴 미감지)인 프레임은 문자열 결과 컬럼을 unknown으로
    ...
    for pos, noise_run in enumerate(runs):
        ...
        # 바로 왼쪽/오른쪽 run이 ANCHOR인지 확인 (붙어있어야만 인정)
        left_anchor = None
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV_FOLDER = "/home/technonia/intern/faceinsight/0205/0211/test_with_ear/csv_kmeans/ver2_csv"
    ...
    if os.path.exists(ROOT_PATH):
        ...
        for idx, folder_name in enumerate(subfolders, 1):
            ...
            try:
                ...
                # 노이즈 제거 적용
                # print("\n노이즈 제거 처리 중...")
                df_final = remove_emotion_noise_v2(df_final)
